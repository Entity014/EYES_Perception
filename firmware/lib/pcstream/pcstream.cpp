...
namespace
{
  ...
  uint32_t g_lastBacklogSend = 0;
  uint8_t *g_drainBuf = nullptr; // PSRAM-backed, allocated once in begin()
  bool g_deferredSession = false;
  bool g_sendSessionComplete = false;
  ...
  bool sendFramed(uint32_t seq, uint8_t flag, const uint8_t *buf, size_t len)
  {
    ...
    if (len && g_client.write(buf, len) != len)
    {
      g_client.stop();
      return false;
    }
  ...
  void tryReconnect()
  {
    if (g_client.connected())
      return;
    uint32_t now = millis();
    if (now - g_lastReconnectAttempt < kReconnectIntervalMs)
      return;
...
namespace pcstream
{
  ...
  void begin()
  {
    ...
    if (!g_drainBuf)
      g_drainBuf = (uint8_t *)malloc(kDrainBufCap);
    if (!g_drainBuf)
      Serial.println("pcstream: drain buffer allocation failed, catch-up disabled");
    tryReconnect();
  }
  ...
  {
    g_spool.clear();
    g_deferredSession = true;
    g_sendSessionComplete = false;
  }

  void queueDeferredFrame(const uint8_t *buf, size_t len)
  {
    if (!g_deferredSession) return;
    const uint32_t seq = g_nextSeq++;
    if (!g_spool.append(seq, buf, len))
      Serial.printf("pcstream: deferred spool write failed, frame %u lost\n", seq);
  }

  void finishDeferredSession()
  {
    if (!g_deferredSession) return;
    g_deferredSession = false;
    g_sendSessionComplete = true;
  }

  void tick()
  {
    tryReconnect();
    if (!g_client.connected() || !g_drainBuf)
      return;
    if (!g_spool.hasPending()) {
      if (g_sendSessionComplete && sendFramed(g_nextSeq++, kFlagSessionComplete, nullptr, 0))
        g_sendSessionComplete = false;
      return;
    }
    uint32_t now = millis();
    if (now - g_lastBacklogSend < kBacklogIntervalMs)
      return;
    ...
    uint32_t seq;
    size_t len;
    if (!g_spool.readNext(seq, g_drainBuf, kDrainBufCap, len))
      return;
